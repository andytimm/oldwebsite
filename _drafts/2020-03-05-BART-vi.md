...
While I'm mostly focused on the modeling step with this post, here's a quick review of the overall process in building a MRP/RRP model. If you want a fuller introduction, Kastellec's [MRP Primer](https://scholar.princeton.edu/jkastellec/publications) is a great starting point.
...
$$
\begin{aligned} \mu_{j} | T_{j} & \sim N\left(\mu_{\mu}, \sigma_{\mu}^{2}\right) \\ \sigma^{2} & \sim I G\left(\frac{\nu}{2}, \frac{\nu \lambda}{2}\right) \end{aligned}
$$
...
Plotting a comparison of the state-level predictions from the two models shows a familiar pattern of pooling. The BART-vi estimates are pulled somewhat towards the grand mean, whereas the ones without varying intercepts are a bit more spread out:
